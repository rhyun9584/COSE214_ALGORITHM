...
void _reheapDown( HEAP *heap, int index){
	...
	while(1){
		if((index * 2 + 1) > heap->last) return; // arr[index] is leaf
...
// heapDelete 함수 호출
tNode *heapDelete( HEAP *heap){
	if(heap->last == -1) return NULL;

	tNode* delete = heap->heapArr[0];
...
tNode *make_huffman_tree( int *ch_freq){
	...
	while(heap->last > 0){
		...
		tNode* new =  newNode(0, min1->freq + min2->freq);
